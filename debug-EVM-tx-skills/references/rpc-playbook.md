...
- 链上失败交易默认先看区块内交易顺序；顺序不大于 30 时用 `cast run`
- 如果链上失败交易的区块内顺序大于 30，就先用 JSON-RPC 获取原交易详情，再用 `cast call`
...
- 当交易在区块内顺序不大于 30 时，使用 `cast run <TX_HASH> --rpc-url <RPC_URL>`
...
- 先用标准 RPC 读取 `transactionIndex`，确认失败交易在所在区块内的顺序
- 如果按用户可读顺序计数大于 30，直接跳过 `cast run`，改用 JSON-RPC 读取原交易详情后执行 `cast call`
...
- 降级或直接分流到 `cast call` 时至少保留这些通过 `eth_getTransactionByHash` 获取的原始参数：`to`、`input`、`from`、`value`、`gas`
...
如果这一步来自“区块内交易顺序大于 30”的直接分流：

- 同样使用失败交易的原始参数
- `--block` 同样固定为失败交易区块号减一
- 执行前同样要验证当前 RPC 的历史状态能力；不支持就更换 Chainlist 候选 RPC
- 输出时明确说明这是基于区块内交易顺序大于 30 的直接 `cast call` 调研路径，且使用的是失败交易前一个块，不是 latest
