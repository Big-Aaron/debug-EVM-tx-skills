...
cp -R ./debug-EVM-tx-skills ~/.claude/skills/
```

安装后目录应当类似：

```text
~/.claude/skills/debug-EVM-tx-skills/
└── SKILL.md
```
...
mkdir -p ./.claude/skills
cp -R /path/to/debug-EVM-tx-skills/debug-EVM-tx-skills ./.claude/skills/
```
...
```text
/debug-EVM-tx-skills 0x1234...abcd on ethereum
```

如果 skill 已正确安装，Claude Code 应该能识别 `/debug-EVM-tx-skills`。
...
```bash
git pull
rm -rf ~/.claude/skills/debug-EVM-tx-skills
cp -R ./debug-EVM-tx-skills ~/.claude/skills/
```
...
2. `debug-EVM-tx-skills/SKILL.md` 仍然是实际安装入口。
3. `debug-EVM-tx-skills/VERSION` 已按语义化版本维护。
4. `debug-EVM-tx-skills/references/` 中的链接没有失效。

## 8. 常见问题

### `/debug-EVM-tx-skills` 不显示

优先检查：

- `~/.claude/skills/debug-EVM-tx-skills/SKILL.md` 是否真实存在
- 目录名是否是 `debug-EVM-tx-skills`
- 文件名是否是 `SKILL.md`
...
说明你可能只更新了仓库根目录，但没有重新复制 `debug-EVM-tx-skills/`。重新复制该目录即可。
